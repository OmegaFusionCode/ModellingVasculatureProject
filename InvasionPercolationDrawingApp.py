...
class InvasionPercolationDrawingApp:
    ...
    def __init__(self, x, y, occ):
        ...
        self.flows = [(ordered(e.a.indices, e.b.indices), v) for e, v in flow_results]
        self.pressures = [(c.indices, v) for c, v in pressure_results]
    ...
    def _draw_vessels_showing_flow(self, flows):
        largest = max((abs(fl) for _, fl in flows))
        for (u, v), fl in flows:
            green = round(255 * ((abs(fl) / largest) ** (1)))
            col = (255, green, 0)

            def coord(p):
                def f(n): return n * self.INTERVAL + self.INTERVAL // 2
    ...
    def _draw_cells_showing_pressure(self, pressures):
        largest = max((pr for _, pr in pressures))

        def coord(p):
            def f(n): return n * self.INTERVAL + self.INTERVAL // 2
            i, j = p
            return f(i), f(j)
        for u, pr in pressures:
            green = round(255 * (pr / largest))
            col = (255, green, 0)
    ...
    def draw(self):
        ...
        if self.mode:
            self._draw_cells_showing_pressure(self.pressures)
        else:
            self._draw_vessels_showing_flow(self.flows)
...
if __name__ == "__main__":
    app = InvasionPercolationDrawingApp(20, 20, 0.5)
    app.run()
